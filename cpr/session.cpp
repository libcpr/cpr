...
void Session::Impl::SetNetworkInterface(const NetworkInterface& network_interface) {
    auto curl = curl_->handle;
    if (curl) {
        curl_easy_setopt(curl, CURLOPT_INTERFACE, network_interface.network_interface.data());
    }
}
...
void Session::SetNetworkInterface(const NetworkInterface& interface) { pimpl_->SetNetworkInterface(interface); }
...
void Session::SetOption(const NetworkInterface& interface) { pimpl_->SetNetworkInterface(interface); }
